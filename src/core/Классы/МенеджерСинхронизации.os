...
Процедура СинхронизироватьХранилищеКонфигурацийСГит(Знач КаталогРабочейКопии, 
		...
		Знач ЗначениеRegExpКомментарияВерсии = ""
		) Экспорт
	...
	Пока СледующаяВерсия <= МаксимальнаяВерсияДляРазбора Цикл
		...
		Если СтрокаВерсии <> Неопределено Тогда
			...
			Если ЗначениеЗаполнено(ЗначениеRegExpКомментарияВерсии) Тогда
				
				Регулярка = Новый РегулярноеВыражение(ЗначениеRegExpКомментарияВерсии);
				
				Если Регулярка.Совпадает() Тогда
					
				Мессенджер = Новый Мессенджер();
				
				Мессенджер.ИнициализацияEmail("Адрес","логин",,,,"Отправитель");
				Мессенджер.ОтправитьСообщениеПоEmail("Получатель","Тема письма","Тело письма");

					ВызватьИсключение "Комментарий к версии не прошел проверку. Версия:"+СледующаяВерсия;
				КонецЕсли;
